...
class ImageToPDFConverter:
    #initialize our self and root
    def __init__(self, root):
        self.root = root
        self.image_paths = [] #convert multiple images
    ...
    def initialize_ui(self):
        title_label = tk.Label(self.root, text="Image to PDF Converter", font=("Helvetoca", 18, "bold"))
    ...
    #function to pick images to be converted
    def select_images(self):
        self.image_paths = filedialog.askopenfilenames(title="Select Images", filetypes=[("Image files", "*.png;*.jpg;*.jpeg")])
        self.update_selected_images_listbox()
    
    def update_selected_images_listbox(self):
        self.selected_images_listbox.delete(0, tk.END) #if there are previous names, delete them
    ...
    def convert_images_to_pdf(self):
        ...
        #resize the images within the document
        for image_path in self.image_paths:
            ...
            pdf.setFillColorRGB(255, 255, 255) #make the page background white
            pdf.rect(0, 0, 612, 792, fill=True)
            pdf.drawInlineImage(img, x_centered, y_centered, width=new_width, height=new_height)#insert images in the page
            pdf.showPage()
